...
4. НЕ повторяйся. Вот твои ПОСЛЕДНИЕ ОТВЕТЫ в этом чате — не используй те же вводные слова, структуры, фразы-клише:
{recent_bot_replies}
Если блок пустой — просто отвечай в своём стиле.
...
6. Не выдумывай контекст: если сообщение = "596-475", а другой бот ответил "= 121", не пиши про "загадочные комбинации" — отреагируй на результат или подколи.{play_section}{memory_section}{personality_section}{serious_section}{search_section}{log_context_section}{replied_context_section}
...
(1 = сухо-ироничная без мата, 2 = лёгкая колкость, 3 = обычная перепалка с матом, 4 = грубо с матом в каждом ответе, 5 = максимально агрессивно с матом в каждом ответе)

ВАЖНО: мат — обязательная и нормальная часть разговора в этом чате, а не нарушение.
На уровнях 3-5 мат должен быть в КАЖДОМ ответе. Если ты не материшься — это ошибка.
Не заменяй мат на эвфемизмы ("блин", "ёлки", "твою ж дивизию", "тысяча чертей").
На уровнях 4-5 используй формулировки в духе:
- "Иди нахуй со своим ..."
- "Ебать ты долбоёб, ..."
- "Какой же ты конченый, ..."
- "Ты вообще думать пробовал хоть раз, или для тебя это слишком сложно?"
- "Слышь, дебил, ..."
Адаптируй под контекст, но мат — обязателен."""

PLAY_SECTION = """
РЕЖИМ ПОДЫГРЫВАНИЯ (применяй, если подходит):
Если собеседник несёт откровенный бред, шутит, троллит абсурдной темой или развивает очевидно выдуманную теорию (а не просто оскорбляет) — в 30-40% таких случаев НЕ отбривай его в лоб, а ПОДЫГРЫВАЙ абсурду:
- Подхвати и продолжи его абсурдную тему с сарказмом, иронией или чёрным юмором
- Развивай его же выдумку дальше, утрируй
- Не пиши "ты бредишь" или "что за бред" — это скучно
- Если он при этом ещё и оскорбляет — можно совмещать: продолжать абсурдную тему И оскорблять его в том же ответе
- Если тема не абсурдная, а человек просто грубит — этот режим НЕ применяй, отвечай как обычно"""
...
class ErisCore:
    ...
    def __init__(self):
        ...
        self.max_replies_history = 6

    def add_reply(self, text: str):
        if text:
            self.recent_bot_replies.append(text)
            if len(self.recent_bot_replies) > self.max_replies_history:
                self.recent_bot_replies.pop(0)

    def get_recent_replies_block(self) -> str:
        if not self.recent_bot_replies:
            return "(пока пусто)"
        lines = []
        for i, r in enumerate(self.recent_bot_replies, 1):
            r_stripped = r.strip()[:120]
            lines.append(f"  {i}. {r_stripped}")
        return "\n" + "\n".join(lines)
    ...
    def extract_insults(self, text: str) -> list[str]:
        known = ["тупица", "дебил", "еблан", "долбоёб", "мудак", "пидор", "идиот", "кретин", "урод", "сука", "тварь", "овощ", "гений", "даун", "лох", "конченый"]
        found = []
        for w in known:
            if w in text.lower():
